...
from boltz_extension.steering.schedules import (
...
def get_potentials(steering_args=None, boltz2=True, score_type: Optional[Union[str, List[str]]] = None):
    """
    Get physical potential functions for steering.
    
    Args:
        score_type: Optional specification of which potentials to return.
                   - None: Return all potentials (default, backward compatible)
                   - str: Return specific potential by name or group
                   - List[str]: Return multiple specific potentials
                   - "vdw": Return VDW overlap potential only
                   - "connections": Return connections potential only
                   - "chirality": Return chirality-related potentials
                   - "geometry": Return geometry-related potentials (bonds, angles)
                   - "stereo": Return stereochemistry potentials
                   - "template": Return template reference potential
                   - "contact": Return contact potential
        steering_args: Optional steering arguments for conditional parameter selection
        boltz2: Optional flag for boltz2-specific potentials
    
    Returns:
        List of potential objects
    """
    
    # Define all potentials with their base configurations
    all_potentials = {
        "SymmetricChainCOMPotential": {
            "class": SymmetricChainCOMPotential,
            "base_params": {
                "guidance_interval": 4,
                "guidance_weight": 0.5,
        ...
        "TemplateReferencePotential": {
            "class": TemplateReferencePotential,
            "base_params": {
                "guidance_interval": 2,
                "guidance_weight": 0.1,
                "resampling_weight": 1.0,
        ...
        "ContactPotentital": {
            "class": ContactPotentital,
            "base_params": {
                "guidance_interval": 4,
                "guidance_weight": PiecewiseStepFunction(thresholds=[0.25, 0.75], values=[0.0, 0.5, 1.0]),
                "resampling_weight": 1.0,
    ...
    # Define potential groups for convenience
    potential_groups = {
        "vdw": ["VDWOverlapPotential"],
        "connections": ["ConnectionsPotential"],
        "chirality": ["ChiralAtomPotential", "StereoBondPotential", "PlanarBondPotential"],
        "geometry": ["PoseBustersPotential", "ConnectionsPotential"],
        "stereo": ["StereoBondPotential", "PlanarBondPotential"],
        "symmetry": ["SymmetricChainCOMPotential"],
        "template": ["TemplateReferencePotential"],
        "contact": ["ContactPotentital"],
        "all": list(all_potentials.keys()),
    }
    
    # Determine which potentials to include
    if score_type is None:
        # Default behavior: return all potentials (backward compatible)
        selected_names = list(all_potentials.keys())
    elif isinstance(score_type, str):
        if score_type in potential_groups:
            selected_names = potential_groups[score_type]
        elif score_type in all_potentials:
            selected_names = [score_type]
        else:
            raise ValueError(f"Unknown score_type: {score_type}. "
                           f"Available types: {list(potential_groups.keys())} or "
                           f"exact names: {list(all_potentials.keys())}")
    elif isinstance(score_type, list):
        selected_names = []
        for name in score_type:
            if name in potential_groups:
                selected_names.extend(potential_groups[name])
            elif name in all_potentials:
        ...
        raise TypeError(f"score_type must be None, str, or List[str], got {type(score_type)}")
    
    # Apply steering_args logic if provided
    if steering_args is not None:
        # Filter potentials based on steering conditions
        if not (steering_args["fk_steering"] or steering_args["physical_guidance_update"]):
            selected_names = []
        
        # Add boltz2-specific potentials if conditions are met
        if boltz2 and (steering_args["fk_steering"] or steering_args["contact_guidance_update"]):
            boltz2_potentials = ["ContactPotentital", "TemplateReferencePotential"]
            selected_names.extend([p for p in boltz2_potentials if p not in selected_names])
    
    # Create potential instances with conditional parameters
    potentials = []
    for name in selected_names:
        if name not in all_potentials:
            continue
            
        potential_config = all_potentials[name]
        params = potential_config["base_params"].copy()
        
        # Apply steering_args conditional logic if provided
        if steering_args is not None:
            contact_guidance = steering_args.get("contact_guidance_update", False)
            
            # Conditionally set guidance_weight based on contact_guidance_update
            if "guidance_weight" in params:
                if isinstance(params["guidance_weight"], (float, int)):
                    params["guidance_weight"] = params["guidance_weight"] if contact_guidance else 0.0
                # For PiecewiseStepFunction, keep as is since it's already conditional
        
        potentials.append(potential_config["class"](parameters=params))
...
class GuidanceConfig:
    ...
    physical_guidance_update: Optional[bool] = None  # renamed from guidance_update
