...
from boltz.model.layers.confidence_utils import (
    ...
    compute_ptms,
    compute_mean_interface_ptm,
    compute_protein_mean_interface_ptm,
)
...
class ConfidenceHeads(nn.Module):
    ...
    def forward(
        ...
        if len(prob_contact.shape) > len(token_pad_pair_mask.shape):
            prob_contact = prob_contact.squeeze(-1)
        
        if token_pad_pair_mask.shape[0] != prob_contact.shape[0]:
            if prob_contact.shape[0] == 1 and token_pad_pair_mask.shape[0] > 1:
                prob_contact = prob_contact.expand(token_pad_pair_mask.shape[0], -1, -1)
        ...
        mean_iptm = compute_mean_interface_ptm(
            pae_logits, x_pred, feats, multiplicity
        )
        out_dict["mean_iptm"] = mean_iptm

        protein_mean_iptm = compute_protein_mean_interface_ptm(
            pae_logits, x_pred, feats, multiplicity
        )
        out_dict["protein_mean_iptm"] = protein_mean_iptm
        out_dict["ligand_mean_iptm"] = ligand_iptm.mean(dim=-1)
