...
from boltz.model.modules.diffusionv2_extend import AtomDiffusion  # use new diffusion module
...
class Boltz2(LightningModule):
    ...
    def __init__(
        ...
        interface_steering_args: Optional[dict] = None,
        ...
        self.interface_steering_args = interface_steering_args
        ...
        if self.confidence_prediction or self.interface_steering_args:
    ...
    def forward(
        ...
        with torch.set_grad_enabled(
            ...
            if (
                ...
                if self.interface_steering_args:
                    self.structure_module.set_confidence_module(self.confidence_module)
                    
                    # Setup trajectory recording if enabled
                    if self.interface_steering_args.get("enable_trajectory_recording", False):
                        vis_output_dir = self.interface_steering_args.get("visualization_output_dir")
                        self.structure_module.setup_trajectory_recording(
                            output_dir=self.interface_steering_args.get("trajectory_output_dir"),
                            save_coordinates=self.interface_steering_args.get("save_coordinates", True),
                            save_confidence_scores=self.interface_steering_args.get("save_confidence_scores", True),
                            enable_visualization=self.interface_steering_args.get("enable_visualization", False),
                ...
                with torch.autocast("cuda", enabled=False):
                    ...
                    if self.interface_steering_args:
                        confidence_kwargs = {
                            ...
                            "s": s,
                            "z": z,
                            "pred_distogram_logits": pdistogram,
                        }
                    
                        network_kwargs["confidence_kwargs"] = confidence_kwargs
                    ...
                    struct_out = self.structure_module.sample(
                        ...
                        interface_steering_args=self.interface_steering_args,
                        **network_kwargs
                    )
