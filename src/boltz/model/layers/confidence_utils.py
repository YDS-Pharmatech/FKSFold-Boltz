...
def compute_mean_interface_ptm(logits, x_preds, feats, multiplicity):
    """
    Compute average interface pTM score
    Based on chai's mean_interface_ptm implementation, calculates average pTM score for all inter-chain interactions
    """
    # Reuse existing computation logic
    _, mask_collinear_pred = compute_frame_pred(
        x_preds, feats["frames_idx"], feats, multiplicity, inference=True
    )
    ...
    bin_width = 32.0 / num_bins
    pae_value = torch.arange(
        start=0.5 * bin_width, end=32.0, step=bin_width, device=logits.device
    ).unsqueeze(0)
    N_res = mask_pad.sum(dim=-1, keepdim=True)
    tm_value = tm_function(pae_value, N_res).unsqueeze(1).unsqueeze(2)
    probs = nn.functional.softmax(logits, dim=-1)
    ...
    unique_asyms_list = torch.unique(asym_id).tolist()
    n_chains = len(unique_asyms_list)
    
    if n_chains < 2:
        # Return 0 if less than 2 chains
        return torch.zeros_like(tm_expected_value[:, 0, 0])
    
    # Calculate interface ptm scores for all inter-chain interactions
    interface_scores = []
    
    for i, asym1 in enumerate(unique_asyms_list):
        for j, asym2 in enumerate(unique_asyms_list):
            if i != j:  # Exclude intra-chain interactions
                # Create mask for inter-chain interactions
                pair_mask_iptm = (
                    maski[:, :, None]
                    * (asym_id[:, None, :] == asym1)
                    * (asym_id[:, :, None] == asym2)
                ...
                if torch.sum(pair_mask_iptm) > 0:
                    chain_iptm = torch.max(
                        torch.sum(tm_expected_value * pair_mask_iptm, dim=-1)
                        / (torch.sum(pair_mask_iptm, dim=-1) + 1e-5),
                        dim=1,
                    ).values
                    interface_scores.append(chain_iptm)
    
    if len(interface_scores) > 0:
        # Calculate average score for all inter-chain interactions
        mean_interface_score = torch.mean(torch.stack(interface_scores), dim=0)
    else:
        mean_interface_score = torch.zeros_like(tm_expected_value[:, 0, 0])
    
    return mean_interface_score


def compute_protein_mean_interface_ptm(logits, x_preds, feats, multiplicity):
    """
    Compute average interface pTM score for protein-protein interactions only
    Based on chai's protein_mean_interface_ptm implementation
    """
    # Reuse existing computation logic
    _, mask_collinear_pred = compute_frame_pred(
        x_preds, feats["frames_idx"], feats, multiplicity, inference=True
    ...
    # Get molecular type information
    ...
    for asym in unique_asyms_list:
        chain_mask = (asym_id == asym) * mask_pad
        if chain_mask.any():
            chain_mask_bool = chain_mask.bool()
            chain_protein_tokens = is_protein_token[chain_mask_bool]
            # Check if this chain is predominantly protein
            if chain_protein_tokens.sum() / chain_mask.sum() > 0.5:
                protein_chains.append(asym)
    
    if len(protein_chains) < 2:
        # Return 0 if less than 2 protein chains
        return torch.zeros_like(tm_expected_value[:, 0, 0])
    
    # Calculate interface ptm scores for protein-protein interactions
    protein_interface_scores = []
    
    for i, asym1 in enumerate(protein_chains):
        for j, asym2 in enumerate(protein_chains):
            if i != j:  # Exclude intra-chain interactions
                # Create mask for protein-protein inter-chain interactions
                pair_mask_iptm = (
                    maski[:, :, None]
                    * (asym_id[:, None, :] == asym1)
                    * (asym_id[:, :, None] == asym2)
                ...
                if torch.sum(pair_mask_iptm) > 0:
                    ...
                    protein_interface_scores.append(chain_iptm)
    
    if len(protein_interface_scores) > 0:
        # Calculate average score for protein-protein inter-chain interactions
        protein_mean_interface_score = torch.mean(torch.stack(protein_interface_scores), dim=0)
    else:
        protein_mean_interface_score = torch.zeros_like(tm_expected_value[:, 0, 0])
    
    return protein_mean_interface_score
