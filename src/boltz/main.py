...
from boltz_extension.steering.base import BoltzInterfaceSteeringConfig
...
def get_cache_path() -> str:
    ...
    return str(Path("./downloads").expanduser())


@rank_zero_only
def print_confidence_scores(predictions_dir: Path, manifest: Manifest) -> None:
    """Print confidence scores from prediction results.
    ...
    manifest : Manifest
        Manifest containing record information
    """
    click.echo("\n" + "="*60)
    click.echo("Confidence Scores Summary")
    click.echo("="*60)
    
    for record in manifest.records:
        record_dir = predictions_dir / record.id
        if not record_dir.exists():
            continue
            
        # Find confidence JSON files
        confidence_files = sorted(record_dir.glob("confidence_model_*.json"))
        ...
        for conf_file in confidence_files:
            try:
                ...
                model_rank = conf_file.stem.split("_")[-1]
                
                click.echo(f"  Model rank {model_rank}:")
                click.echo(f"    Overall confidence score: {conf_data.get('confidence_score', 'N/A'):.4f}")
                click.echo(f"    Complex pLDDT: {conf_data.get('complex_plddt', 'N/A'):.4f}")
                if conf_data.get('ptm') is not None:
                    click.echo(f"    PTM score: {conf_data.get('ptm', 'N/A'):.4f}")
                if conf_data.get('iptm') is not None:
                    click.echo(f"    iPTM score: {conf_data.get('iptm', 'N/A'):.4f}")
                if conf_data.get('complex_pde') is not None:
                    click.echo(f"    Complex PDE: {conf_data.get('complex_pde', 'N/A'):.4f}")
                
                # Print chain-specific scores if available
                if conf_data.get('chains_ptm'):
                    chain_scores = conf_data['chains_ptm']
                    chain_str = ", ".join([f"Chain {k}: {v:.4f}" for k, v in chain_scores.items()])
                    click.echo(f"    Intra-chain PTM: {chain_str}")
                    
            except (json.JSONDecodeError, IOError) as e:
                click.echo(f"  Warning: Failed to read {conf_file.name}: {e}")
...
# works with `inference_with_steering.py`
...
def process_inputs(
    ...
    boltz2: bool, optional
        Whether to use Boltz2, by default True.
    ...
    # Load CCD - always use Boltz2 format
...
@click.option(
    ...
    help=(
        ...
        "Default is ./downloads, or $BOLTZ_CACHE if set."
    ),
...
@click.option(
    "--num_particles",
    type=int,
    help="Number of particles for steering. Default is 3.",
    default=3,
)
@click.option(
    "--fk_lambda",
    type=float,
    help="Lambda parameter for FK steering. Default is 4.0.",
    default=4.0,
)
@click.option(
    "--fk_resampling_interval",
    type=int,
    help="Resampling interval for FK steering. Default is 3.",
    default=3,
)
@click.option(
    "--num_gd_steps",
    type=int,
    help="Number of gradient descent steps for steering. Default is 20.",
    default=20,
)
@click.option(
    "--use_interface_steering",
    is_flag=True,
    help="Whether to use interface confidence steering. Default is False.",
)
@click.option(
    "--interface_scoring_type",
    type=str,
    help="Interface scoring type (iptm, ptm, mean_iptm, etc.). Default is iptm.",
    default="iptm",
)
@click.option(
    "--interface_lambda",
    type=float,
    help="Lambda parameter for interface steering. Default is 2.0.",
    default=2.0,
)
@click.option(
    "--interface_resampling_interval",
    type=int,
    help="Resampling interval for interface steering. Default is 5.",
    default=5,
)
@click.option(
    "--interface_gd_steps",
    type=int,
    help="Number of gradient descent steps for interface steering. Default is 10.",
    default=10,
)
@click.option(
    "--interface_guidance",
    is_flag=True,
    help="(EXPERIMENTAL/DISABLED) Whether to use interface confidence gradient guidance. Default is False.",
)
@click.option(
    "--interface_guidance_strength",
    type=float,
    help="(EXPERIMENTAL/DISABLED) Strength of interface confidence gradient guidance. Default is 0.1.",
    default=0.1,
)
@click.option(
    "--model",
    default="boltz2",
    type=click.Choice(["boltz2"]),
...
def predict(*args, **kwargs):
    """Click wrapper for predict_core."""
    return predict_core(*args, **kwargs)

def predict_core(  # noqa: C901, PLR0915, PLR0912
    data: str,
    out_dir: str,
    cache: str = "./downloads",
    ...
    use_interface_steering: bool = False,
    interface_scoring_type: str = "mean_iptm",
    interface_lambda: float = 2.0,
    interface_resampling_interval: int = 5,
    interface_gd_steps: int = 10,
    interface_guidance: bool = False,
    interface_guidance_strength: float = 0.1,
    ...
    # Force diffusion_samples to 1
    if diffusion_samples > 1:
        msg = "diffusion_samples > 1 is not supported when using steering. Setting to 1."
        click.echo(msg)

    diffusion_samples = 1
    max_parallel_samples = None
    
    # Check if Boltz1 is selected (not supported)
    if model == "boltz1":
        msg = "Boltz1 is no longer supported. Please use --model boltz2"
    ...
    # Set up model parameters - only Boltz2 is supported
    ...
    msa_args = MSAModuleArgs(
        ...
        use_paired_feature=True,  # Always True for Boltz2
    )
    ...
    trainer = Trainer(
        ...
        devices=devices,
        precision="bf16-mixed",  # Always use bf16-mixed for Boltz2
    )
    ...
    if filtered_manifest.records:
        ...
        steering_args.num_gd_steps = num_gd_steps

        # Interface steering configuration
        interface_steering_args = BoltzInterfaceSteeringConfig()
        interface_steering_args.interface_steering=use_interface_steering
        interface_steering_args.interface_scoring_type=interface_scoring_type
        interface_steering_args.interface_lambda=interface_lambda
        interface_steering_args.interface_resampling_interval=interface_resampling_interval
        interface_steering_args.interface_gd_steps=interface_gd_steps
        interface_steering_args.interface_guidance=interface_guidance
        interface_steering_args.interface_guidance_strength=interface_guidance_strength
        interface_steering_args.num_particles=num_particles
        
        # Warning for experimental interface guidance
        if interface_guidance:
            click.echo("⚠️  WARNING: Interface gradient guidance is currently experimental and may not function correctly.")
            click.echo("   This feature is under development and gradient computation may fail.")
            click.echo("   Consider using only interface_steering without guidance for stable results.")
        # Trajectory recording configuration
        interface_steering_args.enable_trajectory_recording=enable_trajectory_recording
        interface_steering_args.trajectory_output_dir=Path(trajectory_output_dir) if trajectory_output_dir else None
        interface_steering_args.save_coordinates=save_coordinates
        interface_steering_args.save_confidence_scores=save_confidence_scores
        interface_steering_args.enable_visualization=enable_visualization
        interface_steering_args.visualization_output_dir=Path(visualization_output_dir) if visualization_output_dir else None

        # TODO: fix AtomDiffusion Confidence Head Call
        model_module = Boltz2.load_from_checkpoint(
            ...
            interface_steering_args=asdict(interface_steering_args),
        )
        model_module.eval()
    ...
    if any(r.affinity for r in manifest.records):
        ...
        interface_steering_args = BoltzInterfaceSteeringConfig()
        interface_steering_args.interface_steering=False
    ...
    print_confidence_scores(out_dir / "predictions", manifest)
